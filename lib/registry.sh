...
feature_remove() {
    ...
    # No custom remove hook → per-feature removal is NOT supported on LiteSpeed.
    # (The previous default here was nginx-specific — conf.d / sites-enabled /
    # vhost.d — copied from nginx-optimizer and never valid for LiteSpeed, where
    # changes land in OLS .conf blocks, Apache includes and .htaccess.) The
    # supported, verified uninstall path is a full backup rollback, which
    # restores the exact pre-change state via checksum verify + health check.
    # A feature MAY still opt in to clean removal by defining
    # feature_remove_custom_<id> (handled above).
    echo "ERROR: per-feature removal is not supported — restore the pre-optimization backup instead: litespeed-optimizer rollback --list, then litespeed-optimizer rollback <timestamp>" >&2
    return 1
}
